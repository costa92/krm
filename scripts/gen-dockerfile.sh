...
if [ $# -ne 2 ];then
    krm::log::error "Usage: gen-dockerfile.sh ${DOCKERFILE_DIR} ${IMAGE_NAME}"
    exit 1
fi
...
# 通用配置
KRM_ALL_IN_KRM_IMAGE_NAME=krm-allinone
KRM_ENV_FILE=${KRM_ENV_FILE:-${KRM_ROOT}/manifests/env.local}
