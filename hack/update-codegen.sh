...
function codegen::applyconfigs() {
    ...
    krm::util::read-array ext_apis < <(
        ...
        git_find -z ':(glob)pkg/apis/**/*types.go' \
            | while read -r -d $'\0' F; do dirname "${KRM_MODULE_NAME}/${F}"; done \
            | sort -u)
    ext_apis+=("${EXTRA_GENERATE_PKG[@]}")
    ext_apis+=("k8s.io/apimachinery/pkg/apis/meta/v1" "k8s.io/api/autoscaling/v1")

    krm::log::status "Generating apply-config code for ${#ext_apis[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: running applyconfiguration-gen for:"
    ...
    applyconfiguration-gen \
        ...
        --go-header-file "${BOILERPLATE_FILENAME}" \
        --output-dir "${KRM_ROOT}/pkg/generated/applyconfigurations" \
        --output-pkg "${APPLYCONFIG_PKG}" \
        "${ext_apis[@]}" \
        "$@"

    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "Generated apply-config code"
    fi
}
...
function codegen::defaults() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: finding all +k8s:defaulter-gen tags"
    fi
    local tag_dirs=()
    krm::util::read-array tag_dirs < <( \
        grep -l --null '+k8s:defaulter-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: found ${#tag_dirs[@]} +k8s:defaulter-gen tagged dirs"
    fi
    ...
    krm::log::status "Generating defaulter code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: running defaulter-gen for:"
        for dir in "${tag_dirs[@]}"; do
            krm::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "Generated defaulter code"
    fi
}
...
function codegen::conversions() {
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: finding all +k8s:conversion-gen tags"
    fi
    local tag_dirs=()
    krm::util::read-array tag_dirs < <(\
        grep -l --null '^// *+k8s:conversion-gen=' "${ALL_K8S_TAG_FILES[@]}" \
            ...
            | sort -u)
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: found ${#tag_dirs[@]} +k8s:conversion-gen tagged dirs"
    fi
    ...
    krm::log::status "Generating conversion code for ${#tag_pkgs[@]} targets"
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "DBG: running conversion-gen for:"
        for dir in "${tag_dirs[@]}"; do
            krm::log::status "DBG:     $dir"
        done
    fi
    ...
    if [[ "${DBG_CODEGEN}" == 1 ]]; then
        krm::log::status "Generated conversion code"
    fi
}
